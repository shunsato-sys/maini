...
export function suggest(state:State,mood:string,food:string,max:number,offset=0):Meal[]{
 const pool=[...state.recipes,...standards].filter((r,i,all)=>all.findIndex(x=>x.name.trim()===r.name.trim()&&x.kind===r.kind)===i);const recent=state.history.slice(0,5).map(h=>h.meal.main.name);const terms=food.split(/[,、\s]+/).filter(Boolean);
 ...
 const start=offset%mains.length||0;return [...mains.slice(start),...mains.slice(0,start)].slice(0,3).map((main,i)=>{const sides=pool.filter(r=>r.kind==='side'&&r.minutes<=max);const side=sides[(i+offset)%sides.length]||null;const soup=mood==='tired'?null:pool.find(r=>r.kind==='soup'&&r.minutes<=max)||null;return {id:crypto.randomUUID(),main,side,soup,reason:recent.includes(main.name)?'最近も作った定番。気分に合わなければ入れ替えよう。':state.recipes.some(r=>r.id===main.id)?'レシピ帳から選びました。最近の献立との重複を控えています。':'まずは定番の献立から。レシピを保存すると、あなたの料理を優先します。',source:state.recipes.some(r=>r.id===main.id)?'saved':'standard'} as Meal});
}
